...
public sealed class LinqExercises
{
    ...
    {
        var res =
            UniversityData.Students.Join(UniversityData.Enrollments, s => s.Id, e => e.StudentId, (s, e) => $"{s.FirstName}, {s.LastName}, {e.EnrollmentDate:yyyy-MM-dd}");
    ...
    public IEnumerable<string> Task14_AverageGradePerCourse()
    {
        ...
        var res = UniversityData.Enrollments.Where(e => e.FinalGrade != null).Join(UniversityData.Courses, e => e.CourseId, c => c.Id, (e, c) => new { c.Title, e.FinalGrade }).GroupBy(x => x.Title).Select(g => $"{g.Key}, {g.Average(x => x.FinalGrade)}");
    ...
    {
        var res = UniversityData.Students.Join(UniversityData.Enrollments, s => s.Id, e => e.StudentId, (s, e) => new { s.FirstName, s.LastName, e.FinalGrade }).Where(x => x.FinalGrade != null).GroupBy(x => new { x.FirstName, x.LastName }).Select(g => $"{g.Key.FirstName}, {g.Key.LastName}, {g.Max(x => x.FinalGrade)}");
        if (res != null)
        {
            return res;
        }
